        ...
        <div className="p-3 ml-16 text-gray-500 flex space-x-14">
            ...
            <div className="flex justify-center items-center space-x-2" onClick={likeComment}>
                ...
                {user.uid === data?.uid && (<div
                    className="cursor-point hover:text-red-600"
                    onClick={deleteTweet}
                > 
                    <TrashIcon className="w-5"/> 
                </div>)}
            <ChartBarIcon className="w-5 cursor-not-allow "/>
            <UploadIcon className="w-5 cursor-not-allow "/>
        </div>
    </div>
  )
}
    ...
    return(
        <div className="flex space-x-3 p-3 border-gray-700">
            ...
            <div>
                ...
                <span>{text}</span>
                {image && <img className="object-cover rounded-md mt-3 max-h-80 " src={image}/>}
            </div>
        </div>
    )
}